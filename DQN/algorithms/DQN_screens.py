...
class DQN(nn.Module):
    def __init__(self, in_channel, hidden_channels,  out_channel, kernel_sizes, strides,
        ...
        # 1st block
        self.conv1 = nn.Conv2d(in_channel, hidden_channels[0], kernel_size=kernel_sizes[0], stride=strides[0])
        self.bn1 = nn.BatchNorm2d(hidden_channels[0])
        # 2nd block
        self.conv2 = nn.Conv2d(hidden_channels[0], hidden_channels[1], kernel_size=kernel_sizes[1], stride=strides[1])
        self.bn2 = nn.BatchNorm2d(hidden_channels[1])
        # 3rd block
        self.conv3 = nn.Conv2d(hidden_channels[1], hidden_channels[1], kernel_size=kernel_sizes[2], stride=strides[2])
        ...
        convw = conv2d_size_out(conv2d_size_out(conv2d_size_out(observation_space_w,kernel_sizes[0],strides[0]),
                                                kernel_sizes[1],strides[1]),
                                                kernel_sizes[2],strides[2])
        convh = conv2d_size_out(conv2d_size_out(conv2d_size_out(observation_space_h,kernel_sizes[0],strides[0]),
                                                kernel_sizes[1],strides[1]),
                                                kernel_sizes[2],strides[2])
